...
<html>
  ...
  <body class="layui-anim layui-anim-up">
    ...
    <div class="x-body">
      <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="${pageContext.request.contextPath}/reader/search" method="post">
      ...
      <xblock>
        ...
        <%--<button class="layui-btn" onclick="x_admin_show('添加读者','${pageContext.request.contextPath}/reader/addPage',600,400)"><i class="layui-icon"></i>添加</button>--%>
        ...
        <tbody>
          <c:forEach items="${list}" var="reader">
            <tr>
              ...
              <td>${reader.readerSex==1 ? "男":"女"}</td>
              <td>${reader.readerEmail}</td>
              <td>${reader.readerPhone}</td>
              <td>${reader.readerAddTime}</td>
              <td class="td-status">
                <span class="layui-btn layui-btn-danger ${reader.readerStatus==3 ?"layui-btn-disabled":""} layui-btn-mini">${reader.readerStatus==2 ? "已删除":"已恢复"}</span></td>
              <td class="td-manage">
                <a onclick="member_stop(this,'${reader.readerId}')" href="javascript:;"  title="${reader.readerStatus==2? "恢复":"删除"}">
      ...
      <div class="page">
        ...
        <div>
          <c:if test="${pageInfo.hasPreviousPage}">
            <a class="prev" href="${pageContext.request.contextPath}/reader/list?page=${pageInfo.prePage}">&lt;&lt;</a>
          </c:if>
          <c:forEach items="${pageInfo.navigatepageNums}" var="p">
            ...
            <c:if test="${pageInfo.pageNum!=p}">
              <a class="num" href="${pageContext.request.contextPath}/reader/list?page=${p}">${p}</a>
            </c:if>
          </c:forEach>
          <c:if test="${pageInfo.hasNextPage}">
            <a class="next" href="${pageContext.request.contextPath}/reader/list?page=${pageInfo.nextPage}">&gt;&gt;</a>
          </c:if>
        </div>
      </div>
    ...
    <script>
      ...
      function delAll (argument) {
        ...
        layer.confirm('确认要恢复吗？',function(index){
            ...
            $.ajax({
                type:"post" ,
                url:"${pageContext.request.contextPath}/reader/huifuAll",
                data:{ids:data},
                dataType:"json",
                success:function (data) {
